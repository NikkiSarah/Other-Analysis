...
def read_tweet_data(tweet_file):
    ...
    for line in tweet_file:
        ...
        data.append({'id': id, 'text': text, 'language': language, 'place': place, 'location': location})

    out_dict = {'id': [], 'text': [], 'language': [], 'place': [], 'location': []}
...
def process_location_info(tweet_dict, state_dict, state_dict_r):
    ...
    code1 = [loc.split(",")[-1].strip().upper() if bool(loc) and re.match(r'.*, [A-Z]{2}$', loc) else None
            for loc in location_data]
    code2 = [loc if loc in state_dict.keys() else None for loc in code1]
    name1 = [loc.split(",")[0].strip().title() if bool(loc) else None for loc in location_data]
    name2 = [loc if loc in state_dict.values() else None for loc in name1]
    code = [state_dict_r.get(loc, None) for loc in name2]    # combine the two
    state_code = [(sc1 if sc1 else sc2 if sc2 else None) for sc1, sc2 in zip(code2, code)]
